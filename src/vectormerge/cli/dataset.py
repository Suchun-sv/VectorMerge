...
from vectormerge.core import DatasetPrepareSpec, RunOptions, prepare_dataset

# Initialize dataset command group
dataset_app = typer.Typer(
    help="""Download and manage datasets,
...
def download_dataset(
    ...
    data_path: Optional[Path] = typer.Option(
        cli_defaults.data_path, "--data-path", help="Path to save datasets"
    ),
    model_mode: str = typer.Option(
        "none",
        "--model-mode",
        "-m",
        help="Embedding preparation mode: none|all|pair",
    ),
    ...
    for d in datasets:
        if d not in SUPPORTED_DATASETS:
            display_error_and_exit(
                f"Dataset '{d}' not supported. Use 'vectormerge dataset list' to see available datasets."
            )
    ...
    for item in datasets:
        run = prepare_dataset(
            DatasetPrepareSpec(
                ...
                overrides={"data_path": str(data_path)},
                options=RunOptions(force=force, verbose=verbose),
            )
        )
        if run.status == "success":
            success_count += 1
        else:
            failed_datasets.append(item)
...
def list_datasets(
    ...
    data_path: Path = typer.Option(
        cli_defaults.data_path, "--data-path", help="Path to data directory"
    ),
):
    ...
    rprint(
        "[cyan]vectormerge dataset download -d <name>[/cyan] - Download specific dataset"
    )
...
def dataset_info(
    ...
    if dataset not in SUPPORTED_DATASETS:
        display_error_and_exit(
            f"Dataset '{dataset}' not supported. Use 'vectormerge dataset list' to see available datasets."
        )
    ...
    if not dataset_path.exists():
        display_error_and_exit(
            f"Dataset '{dataset}' not found. Download it first with 'vectormerge dataset download -d {dataset}'"
        )
    ...
    try:
        ...
        # Calculate size
        size = format_size(
            sum(f.stat().st_size for f in dataset_path.rglob("*") if f.is_file())
        )
...
def _show_download_results(
    success_count: int, failed_datasets: List[str], data_path: Path
) -> None:
