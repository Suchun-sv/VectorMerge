...
class ClusteringStrategy(ABC):
    ...
    def cluster_reference(self, embeddings: np.ndarray, 
        ...
        Returns:
            Tuple of (cluster_data_list, labels)
        """
        pass
    ...
    def assign_to_clusters(self, embeddings: np.ndarray, target_indices: np.ndarray,
        ...
        """Assign target points to clusters.
        
        Args:
            embeddings: All embeddings
            target_indices: Indices of target points
            cluster_data_list: Existing cluster data
            reference_embeddings: Reference embeddings for distance calculation
            
        Returns:
            Updated cluster data list
        """
        pass
    
    def cluster(self, embeddings: np.ndarray, reference_indices: np.ndarray,
               target_indices: np.ndarray) -> List[ClusterData]:
        """Full clustering pipeline.
        
        Args:
            embeddings: All embeddings
            reference_indices: Reference point indices
            target_indices: Target point indices
            
        Returns:
            List of cluster data with assigned points
        """
        # Step 1: Cluster reference points
        cluster_data_list, labels = self.cluster_reference(embeddings, reference_indices)
        
        # Step 2: Assign target points to clusters
        updated_clusters = self.assign_to_clusters(
            embeddings, target_indices, cluster_data_list, embeddings[reference_indices]
        )
        
        logger.info(f"Clustering completed: {len(updated_clusters)} clusters")
